...
import { Briefcase, Calendar, CheckSquare, ArrowRight, UserCheck, ShieldAlert, Award, LogOut, CheckCircle, Clock, Loader2, AlertCircle } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import * as workerService from '../../services/workerService';

export const LaborDashboard = () => {
  const { user } = useAuth();
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to retrieve jobs and worker registry information.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApplyJob = async (jobId) => {
    if (workerId) {
      try {
        await workerService.applyForJob(jobId, workerId);
  ...
  const handleAcceptJobApplication = async (appId) => {
    if (workerId) {
      try {
        await workerService.assignWorkerToJob(appId, workerId);
        await fetchDashboardData();
      } catch (err) {
  ...
  const handleResign = async () => {
    if (activeJobData) {
      try {
        await workerService.unassignWorker(activeJobData.id);
        await fetchDashboardData();
      } catch (err) {
        alert(err.message || 'Failed to resign from job.');
      }
    }
  };
  ...
  if (loading) {
    return (
      <div className="glass p-12 rounded-3xl border border-emerald-100/50 shadow-sm flex flex-col items-center justify-center min-h-[350px]">
        ...
        <p className="text-xs font-extrabold text-slate-400">Loading laborer workspace...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="glass p-12 rounded-3xl border border-red-100 shadow-sm flex flex-col items-center justify-center min-h-[350px] text-red-500 bg-red-50/10">
        <AlertCircle size={40} className="mb-2" />
        <p className="text-xs font-extrabold">Failed to load workspace data</p>
